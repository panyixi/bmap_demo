	...
	window.Swiper = function(e, t) {
		...
		if (f = "undefined" == typeof a ? window.Dom7 || window.Zepto || window.jQuery : a, f && (h.container = f(e), 0 !== h.container.length)) {
			...
			if (h.onTouchStart = function(e) {
					...
					jQuery("#cart").attr("gc_id",jQuery(".swiper-slide-active").attr("gc_id"));
					var mark_id=jQuery(".swiper-slide-active").attr("data-swiper-slide-index");
					sessionStorage.setItem("mark_id",mark_id); 
					jQuery("#collector").attr("gc_id",jQuery(".swiper-slide-active").attr("gc_id"));
				      jQuery.ajax({type:'POST',
				          url: "/social/home/get_favoriteFlag.htm",
				          data:{"goods_id":jQuery(".swiper-slide-active").attr("gc_id")},
				          dataType: "json",
				          success:function(data){ 
				          if(data.favoriteFlag==1)
				          {
				              jQuery(".unselect").css("display","none");
				              jQuery(".select").css("display","block");
				          }
				          else
				        	  {
				        	  jQuery(".select").css("display","none");
				              jQuery(".unselect").css("display","block");
				        	  }
				            // var out_put=data.dataone;
				            // jQuery(".heading").append(out_put);
				          }
				        });
